...
const labels = {long_name:'Nome do dispositivo',short_name:'Nome curto',is_licensed:'Modo radioamador',is_unmessagable:'Ocultar opção de mensagem',
  ...
  config_ok_to_mqtt:'Permitir encaminhamento para MQTT',tx_enabled:'Transmissão habilitada',tx_power:'Potência configurada (dBm)',channel_num:'Slot de frequência',bandwidth:'Largura de banda',spread_factor:'Fator de espalhamento',coding_rate:'Taxa de codificação',
...
const hints = {gps_mode:'Controla o GPS. ENABLED permite buscar satélites; coordenadas só aparecem após obter uma posição válida.',
  ...
  config_ok_to_mqtt:'Permite que outro rádio encaminhe seus pacotes para MQTT. Não ativa o módulo MQTT deste aparelho.',
...
let closing = '';
let csrf = '', schema = [], state = {session_active:false,connected:false,sections:{},nodes:[],events:[]}, ports = [], page = 'overview', selected = '', drafts = {}, busy = false, applying = false, jsonMode = false, fieldSearch = '', nodeSearch = '', errors = {}, toastTimer;
...
function needsReconnect(){return state.session_active&&!state.demo&&state.connection_mode==='desktop'&&(!state.connected||state.active_phase!=='active');}
...
if(needsReconnect())return `<div class="status-strip"><span><b>Rádio desconectado.</b> Seus rascunhos estão preservados. Reconecte para consultar ou salvar.</span>${button('reconnect','Reconectar ao rádio','plug','primary')}</div>`;
...
function statusStrip(){return snapshotNotice()+ `<div class="status-strip ${state.demo?'demo':''}">${icon(state.demo?'code':'shield')}<span>${state.demo?'<b>Ambiente de demonstração.</b> Todos os dados são fictícios; as alterações são simuladas.':writesAllowed()?'<b>Gravação habilitada.</b> Use Salvar alterações para gravar a seção e conferir o resultado.':'<b>Você está em modo de leitura.</b> Explore e prepare rascunhos. A gravação no dispositivo está bloqueada.'}</span><span class="end">${state.device?`${state.device.write_packets} gravações de configuração`:'ACESSO LOCAL'}</span></div>`;}
function render(){
  if(closing){
...
function renderFields(fields,data,prefix=''){
  return fields.filter(f=>!f.deprecated).map(f=>{
    ...
    const attr=`data-field="${esc(path)}" data-kind="${f.kind}" data-bits="${f.bits||0}" ${f.readonly||busy||applying?'disabled':''}`;
    ...
    if(f.kind==='bool'&&!f.repeated)return `<label class="switch-row"><span>${title}${hints[f.name]?`<span class="field-hint">${esc(hints[f.name])}</span>`:''}</span><input type="checkbox" class="switch" ${attr} ${value===true?'checked':''} aria-label="${esc(label)}"></label>`;
    let control;
...
function draftBar(){
  ...
  if(!dirty(selected))return count?`<p class="export-note">${count} seção(ões) com alterações pendentes. Acesse Rascunhos para continuar.</p>`:'';
  const mayRestart=['config.lora','config.network','config.bluetooth','config.security','config.device'].includes(selected)||selected.startsWith('channel.');
  const hint=!writesAllowed()?'O servidor está em modo somente leitura.':needsReconnect()?'Reconecte ao rádio para salvar. Seu rascunho está preservado.':applying?'Salvando e conferindo no rádio…':mayRestart?'Salvar esta seção pode reiniciar o rádio ou interromper a conexão.':'As alterações serão salvas somente nesta seção.';
  return `<div class="draft-bar"><span class="draft-dot"></span><div><h3>Alterações em ${esc(schema.find(s=>s.id===selected)?.name)}</h3><p>${hint}</p></div><div class="actions">${button('discard','Descartar','close','ghost')}${button('save',applying?'Salvando…':'Salvar alterações','check','primary',errors[selected]||!writesAllowed()||needsReconnect()?'disabled':'')}</div></div>`;
}
function ensureDraft(){if(!drafts[selected])drafts[selected]={values:clone(sectionValue(selected)),original:clone(sectionValue(selected)),revision:state.sections[selected].revision,epoch:state.epoch,nodeId:state.device?.id,rawFields:{},fieldErrors:{}};return drafts[selected];}
...
function nodesPage(){const nodes=state.nodes.filter(n=>(n.name+' '+n.id+' '+n.hardware).toLowerCase().includes(nodeSearch.toLowerCase()));return heading('Os pontos da sua rede.',`${state.nodes.length} registros conhecidos. Um registro não confirma que o nó está online ou ao alcance direto.`,button('sync-state','Atualizar lista','refresh','ghost'))+statusStrip()+`<p class="field-hint">Renovar nó remove o cadastro salvo desse contato, incluindo sua chave pública e favoritos, e solicita os dados novamente. A chave do seu rádio e o histórico do app são preservados. Depende da resposta do outro nó; não garante a entrega de mensagens.</p>${Object.entries(state.node_renewals||{}).map(([id,r])=>`<p role="status" class="field-hint">${esc(r.name)} · ${esc(id)}: ${esc({waiting:'Aguardando dados e chave pública. Mantenha a recepção conectada ou atualize a lista depois.',received:'Chave pública recebida. Você já pode testar uma mensagem direta.',uncertain:'Operação interrompida; o cadastro pode ter sido removido. Atualize a lista antes de repetir.',simulated:'Renovação simulada; nenhum rádio foi acessado.'}[r.status])}</p>`).join('')}<div class="editor-toolbar"><label class="search">${icon('search')}<input id="node-search" placeholder="Buscar nome, ID ou hardware…" aria-label="Buscar nós" value="${esc(nodeSearch)}"></label><span class="muted small">${nodes.length} resultado(s)</span></div><section class="card editor-panel table-wrap"><table class="node-table"><thead><tr><th>DISPOSITIVO</th><th>HARDWARE</th><th>ORIGEM REGISTRADA</th><th>SNR</th><th>SALTOS</th><th>ÚLTIMO REGISTRO</th><th>CADASTRO</th></tr></thead><tbody>${nodes.map(n=>`<tr><td><div class="node-name">${esc(n.name)} ${n.local?'<span class="pill green">Você</span>':''}</div><div class="node-id mono">${esc(n.id)}</div></td><td class="tiny">${esc(n.hardware)}</td><td><span class="pill ${n.local?'green':''}">${n.local?'Local':n.via_mqtt?'MQTT':'Sem marca MQTT'}</span></td><td>${n.snr===null||n.snr===undefined?'—':`${Number(n.snr).toFixed(1)} dB`}</td><td>${esc(n.hops??'—')}</td><td class="small muted">${n.last_heard?esc(new Date(n.last_heard*1000).toLocaleString('pt-BR')):'—'}</td><td>${n.local?'':`<button class="btn ghost" data-renew-node="${esc(n.id)}" ${busy||!writesAllowed()?'disabled':''}>Renovar nó</button>`}</td></tr>`).join('')||'<tr><td colspan="7" class="empty">Nenhum nó para mostrar.</td></tr>'}</tbody></table></section>`;}
...
function bind(){
  ...
  bindActions();
  document.querySelectorAll('[data-renew-node]').forEach(el=>el.onclick=()=>task(async()=>{
    const epoch=state.epoch;
    try{state=await api('nodes/renew',{node_id:el.dataset.renewNode,epoch});}
    finally{state=await api('state');}
  }));
...
const actions={
  ...
  save:saveSettings,
  reconnect:()=>task(async()=>{try{await api('messages/active',{});}finally{state=await api('state');}}),
...
async function saveSettings(){
  const key=selected, draft=drafts[key];
  if(busy||applying||closing||!draft||!dirty(key)||errors[key]||!writesAllowed())return;
  if(needsReconnect())return notify('Reconecte ao rádio antes de salvar. Seu rascunho está preservado.',true);
  const values=clone(draft.values), epoch=state.epoch, nodeId=state.device?.id;
  applying=true;busy=true;render();
  try{
    state=await api('state');
    if(state.epoch!==epoch||state.device?.id!==nodeId||draft.epoch!==epoch||draft.nodeId!==nodeId)throw new Error('A sessão do rádio mudou. O rascunho foi preservado, mas não foi enviado.');
    if(needsReconnect())throw new Error('O rádio desconectou. Reconecte e clique em Salvar alterações novamente.');
    const grant=await api('preview',{section:key,revision:draft.revision,values});
    if(!grant.can_apply)throw new Error('A gravação está bloqueada no servidor.');
    // The click authorizes this exact section. Validation, one-use authorization,
    // device identity checks and readback remain enforced by the backend.
    state=await api('apply',{token:grant.token,confirmation:grant.confirmation});
    if(drafts[key]===draft){delete drafts[key];delete errors[key];}
    notify(state.demo?'Alterações salvas e conferidas no simulador.':'Alterações salvas e conferidas no rádio.');
  }catch(e){
...
async function syncSession(){
  ...
  csrf=session.token;schema=session.schema;state=session.state;
  render();
}
